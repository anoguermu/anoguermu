...
Pos trobar_pos(Mat_pos& pant, Pos p,Pos p_aliat){
  while(pant[p.i][p.j]!=p_aliat){
      p=pant[p.i][p.j];
      if(p_aliat==Pos(0,25,1))cerr<<p<<endl;
...
double Dist_AscenssorgemaProp(pair<int,int> ascenssors, int& dist){
  ...
  if((distmin+dist)<=(distSol_gema)/2)dist+=distmin;
  else dist=9999999;
...
void Mou_Pioners(Moviments& moviments){
  ...
  for(int id : Pioners){
    ...
    if(unit(id).pos.k==0)Mou_Pioners_Cova(id,moviments);
    else Mou_Pioners_fora(id,moviments);
  }
...
void fer(Moviments & moviments){
  ...
  if(round()==105)coutmovs(moviments);
